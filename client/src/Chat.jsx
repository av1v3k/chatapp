...
export default function Chat() {
  ...
  function connectToWS() {
    ...
    //Lost connection and re-connecting
  ...
  function shownOnlinePeople(peopleArray) {
    const people = {};
  ...
  function handleMessage(ev) {
    ...
    // e.data.text().then(msgString => {
    //   console.log(msgString);
    // });
    if ("online" in msgData) {
      shownOnlinePeople(msgData.online);
  ...
  const onlinePeoplerExlOurUser = { ...onlinePeople };
  delete onlinePeoplerExlOurUser[id];
    ...
    <div className="flex h-screen">
      ...
      <div className="bg-white w-1/4 flex flex-col">
        <div className="flex-grow">
          <Logo />
          {Object.keys(onlinePeoplerExlOurUser).map((userid) => {
            return (
              <Contact
                key={userid}
                online={true}
